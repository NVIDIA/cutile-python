...
struct ArrayTypeConstantBuilder {
    ...
    Status update(const Arena& arena, const ArrayRepr& ar,
        ...
        bits &= compute_array_specialization_bits(
                    ...
                    /*infer_shape_one=*/static_shape_dims.empty() && can_specialize_for_shape,
...
static void require_min_cconv(CallConvVersion* current_minimum, CallConvVersion requirement) {
    if (*current_minimum < requirement)
        *current_minimum = requirement;
}
...
static PyPtr parse_array_constraint(ConstantCursor& cursor,
    ...
    for (size_t i = 0; i < arrty.ndim; ++i) {
        if (special_bits.is_shape_one(i)
...
static Status extract_py_list(const DriverApi* driver, PyObject* pyobj,
    ...
    if (!builder.update(helper.arena, *first_repr_res, list_ann.element.static_shape_dims,
                        list_ann.element.static_stride_dims, helper.can_specialize_for_shape))
        return ErrorRaised;
...
static PyPtr parse_list_constraint(ConstantCursor& cursor,
    ...
    PyPtr element = parse_array_constraint(
            cursor, static_shape_dims, static_stride_dims, minimum_cconv);
    if (!element) return {};
...
static PyPtr parse_element_constraint(
    ...
    case ParameterKind::Array:
        return parse_array_constraint(cursor, annotation.array.static_shape_dims,
                                      annotation.array.static_stride_dims, minimum_cconv);
    ...
    case ParameterKind::List:
        return parse_list_constraint(cursor, annotation.list.element.static_shape_dims,
                                     annotation.list.element.static_stride_dims, minimum_cconv);
...
static PyObject* get_parameter_constraints_from_pyargs(PyObject* self, PyObject* args) {
    ...
    if (requested_cconv < CallConvVersion::CutilePython_V2)
