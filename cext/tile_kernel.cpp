...
static ArraySpecializationBits compute_array_specialization_bits(
        ...
        bool infer_shape_one, bool infer_stride_one) {
...
struct ArrayTypeConstantBuilder {
    ...
    Status update(const Arena& arena, const ArrayRepr& ar,
        ...
        // An explicit annotation defines the complete set of shape/stride constants;
        // only infer singleton shapes and unit strides when no corresponding annotation exists.
        bits &= compute_array_specialization_bits(
                    repr, ar.arrty.ndim, ar.arrty.dtype.bits * ar.arrty.dtype.lanes,
                    ar.arrty.index_bitwidth,
                    /*infer_shape_one=*/static_shape_dims.empty(),
                    /*infer_stride_one=*/static_stride_dims.empty()).u64;
...
static PyPtr parse_array_constraint(ConstantCursor& cursor,
    ...
    for (size_t i = 0; i < arrty.ndim; ++i) {
        if (target_cconv >= CallConvVersion::CutilePython_V2
                && special_bits.is_shape_one(i)
                && PyList_GET_ITEM(constant_shape.get(), i) == Py_None)
            PyList_SET_ITEM(constant_shape.get(), i, Py_NewRef(one.get()));

        if (PyList_GET_ITEM(constant_shape.get(), i) != Py_None)
            minimum_cconv = CallConvVersion::CutilePython_V2;
    }
...
static PyPtr parse_list_constraint(ConstantCursor& cursor,
    ...
    PyPtr element = parse_array_constraint(
            cursor, static_shape_dims, static_stride_dims, target_cconv, minimum_cconv);
    if (!element) return {};
...
static PyPtr parse_element_constraint(
    ...
    case ParameterKind::Array:
        return parse_array_constraint(cursor, annotation.array.static_shape_dims,
                                      annotation.array.static_stride_dims, target_cconv,
                                      minimum_cconv);
    ...
    case ParameterKind::List:
        return parse_list_constraint(cursor, annotation.list.element.static_shape_dims,
                                     annotation.list.element.static_stride_dims, target_cconv,
                                     minimum_cconv);
...
static PyPtr parse_param_constraint(
        ConstantCursor& cursor,
    ...
    if (pk.category == ParameterKind::AggregateBegin) {
        ...
        if (minimum_cconv < CallConvVersion::CutilePython_V2)
            minimum_cconv = CallConvVersion::CutilePython_V2;
        ...
        case AggregateArgType::Dataclass:
            minimum_cconv = CallConvVersion::CutilePython_V3;
    ...
    return parse_element_constraint(
            cursor, pk.category, *annotation, identity_constants,
            target_cconv, minimum_cconv);
}

// `target_cconv` limits only automatically inferred constraints. e.g., shape_one.
...
static PyPtr make_signature(ConstantCursor constants,
    ...
    PyPtr parameters = parse_parameter_constraints(
            ...
            /*target_cconv=*/CallConvVersion::CutilePython_V2, minimum_cconv);
...
static PyObject* get_parameter_constraints_from_pyargs(PyObject* self, PyObject* args) {
    ...
    PyPtr parameters = parse_parameter_constraints(
            ...
            py_unwrap<CallingConvention>(cconv).version, minimum_cconv);
