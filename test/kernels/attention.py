...
def fmha_bwd_dq_kernel(Q, K, V, Grad, Delta, Lse, DQ, qk_scale: float, input_pos: int, TILE_D: ConstInt, 
    ...
    delta_i = ct.load(Delta, index=(batch_idx, head_idx, bid_x), shape=(1, 1, TILE_M),
                      allow_tma=allow_tma).reshape((TILE_M, 1))
    do = ct.load(
        Grad, index=(batch_idx, head_idx, bid_x, 0), shape=(1, 1, TILE_M, TILE_D), allow_tma=allow_tma
    ).reshape((TILE_M, TILE_D))  # [TILE_M, TILE_D]
    dq = ct.full((TILE_M, TILE_D), 0., dtype=np.float32) # [TILE_M, TILE_D]
    ...
    for j in range(0, Tc):
        ...
        dskt = ct.full((TILE_M, TILE_D), 0., dtype=np.float32) # [TILE_M, TILE_D]
...
def fmha_bwd_dk_dv_kernel(Q, K, V, Grad, Delta, Lse, DK, DV, qk_scale: float, input_pos: int, TILE_D: ConstInt, 
    ...
    dk = ct.full((TILE_N, TILE_D), 0., dtype=np.float32) # [TILE_N, TILE_D] 
    dv = ct.full((TILE_N, TILE_D), 0., dtype=np.float32) # [TILE_N, TILE_D]
    ...
    for i in range(0, Tr):
        ...
        if CAUSAL:
            # when kv pos could exceed q pos
            mask_start = (input_pos + i * TILE_M) // TILE_N
            # when kv pos could exceed k_seqlen
            mask_start = min(mask_start, k_seqlen // TILE_N)
        else:
            mask_start = k_seqlen // TILE_N
        ...
        # # --- Apply Causal Masking ---
        # if (CAUSAL or not EVEN_K) and bid_x <= mask_start:
        ...
        ptdo = ct.full((TILE_N, TILE_D), 0., dtype=np.float32) # [TILE_N, TILE_D]
        ptdo = ct.mma(pt, do, ptdo) # [TILE_N, TILE_D]
        dv = dv + ptdo # [TILE_N, TILE_D]
        ...
        dstq = ct.full((TILE_N, TILE_D), 0., dtype=np.float32) # [TILE_N, TILE_D]
        dstq = ct.mma(dst, q, dstq) # [TILE_N, TILE_D]
        dk = dk + dstq
